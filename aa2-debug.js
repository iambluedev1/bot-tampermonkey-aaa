...
(function() {
    ...
    var myHandler = function (messages, context) {
        ...
        jQuery.post('https://blackpink-access.com/debug/send-logs-enaiid', { message: prefix + " " + messages[0], level: context.level });
    };
    ...
    function vote(index, token){
        var url = "https://vote-api.aaavietnam2019.com.vn/Vote/Vote?v=1568566578451";
        var method = "POST";
    ...
    function interceptHttp(xhr){
        ...
        if(email.state == 4){
            if(xhr.responseURL.startsWith("https://www.googleapis.com/identitytoolkit/v3/relyingparty/verifyPassword")){
                Logger.debug("Intercept HTTP Request");
                updateState(index, 5);
                if(xhr.status != 200){
    ...
    function launch(){
        ...
        if(email.state == 0){
            ...
            try {vote(index, bearerToken);}catch(e){vote(index, bearerToken);}
